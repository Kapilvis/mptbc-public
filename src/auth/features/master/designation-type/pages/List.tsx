...
export default function List() {
  ...
  const handleToggleStatus = async (item: Master.DesignationTypeList) => {
    try {
      const result = await toggleStatus({
        designationTypeId: item.designationTypeId,
  ...
  return (
    <Page
      header={`${pageTitle}`}
      subHeader="Create and manage designation types for organizational classification."
      showHeaderActions
    >
      <Card>
        <GridPanel
          ...
          columns={[
            ...
            { field: "localName", header: "Local Name" },
            {
              ...
              header: "Status",
              align: "center",
              cell: (row: Master.DesignationTypeList) => (
                <StatusButton
                  value={row.isActive}
          ...
          renderContent={(item: Master.DesignationTypeList) => (
            <Mosaic.Card
              title={item.name}
