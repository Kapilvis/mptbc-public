...
export default function List() {
  ...
  const handleToggleStatus = async (item: Master.NationalityItem) => {
    try {
      ...
      if (result) {
        ToastService.success("Nationality status updated successfully");
      }
    } catch {
      ToastService.error("Failed to update nationality status");
    }
  };
  ...
  return (
    <Page
      header={`${pageTitle}`}
      subHeader="Create and manage nationality data for beneficiary and staff records."
      showHeaderActions
    >
      <Card>
        <GridPanel
          ...
          columns={[
            ...
            { field: "code", header: "Code", align: "center" },
            {
              ...
              header: "Status",
              align: "center",
              cell: (row: Master.NationalityItem) => (
                <StatusButton
                  value={row.isActive}
